...
setwd("G:\\CKDurine\\19.go\\125 genes\\4.GO")       #设置工作目录
rt=read.table("aging_genes_diff_results.txt", header=T, sep="\t", check.names=F)     #读取差异分析的结果文件
...
bar=barplot(kk, showCategory=8, label_format=100, split="ONTOLOGY", color=colorSel, drop=TRUE) + facet_grid(ONTOLOGY~., scale='free')
print(bar)
dev.off()
...
#绘制圈图主体部分
pdf(file="GO.circlize.pdf", width=7, height=7)
...
GOBubble(circ, title = 'Bubble plot', display = 'multiple', bg.col=T, labels=2.5)
dev.off()
